...
//! clips. All route through the configured gateway provider so fallback and
//! provider settings apply consistently to every text tool.

use serde::Deserialize;
use tauri::{command, AppHandle, Emitter};

use ai_kit::ProviderCapability;
use content_kit::{
...
use crate::commands::ai::build_gateway_provider;
...
#[command]
pub async fn content_summary(app: AppHandle, request: ContentRequest) -> Result<SummaryResult, String> {
    let provider = build_gateway_provider(
        &app,
        "summary",
...
#[command]
pub async fn content_chapters(app: AppHandle, request: ContentRequest) -> Result<ChapterList, String> {
    let provider = build_gateway_provider(
        &app,
        "chapters",
        ProviderCapability::Text,
...
#[command]
pub async fn content_youtube_pack(
    app: AppHandle,
    request: ContentRequest,
) -> Result<YouTubePack, String> {
    let provider = build_gateway_provider(
        &app,
        "youtube-pack",
        ProviderCapability::Text,
...
#[command]
pub async fn content_viral_clips(
    app: AppHandle,
    request: ContentRequest,
) -> Result<ViralClipList, String> {
    let provider = build_gateway_provider(
        &app,
        "viral-clips",
        ProviderCapability::Text,
